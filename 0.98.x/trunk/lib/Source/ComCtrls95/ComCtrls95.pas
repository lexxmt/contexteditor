...
uses Messages, Windows, SysUtils, ComCtrls, CommCtrl, Classes, controls,
     Forms, Graphics, typinfo, dialogs, ImgList
     {$ifdef delphi4}
        {$ifNDef BCB3}
        ,imglist
        {$endif}
...
begin
  if (fMouseDownTab=fMouseOverTab) then begin
    TabIndex:=fMouseOverTab;
    if Assigned(FOnChange) then
      FOnChange(SELF);
  end;

  if (fTabShifting and (Button=mbLeft) and (fMouseOverTab<>-1) and (fMouseDownTab<>fMouseOverTab)) then begin
...
begin
  if (fTabShifting) and (ssLeft in Shift) and (fMouseOverTab<>fMouseDownTab) then begin
