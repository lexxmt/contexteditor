...
procedure Register;
Begin
  RegisterComponents('My Controls', [TLibTidy]);
end;
...
begin
  AssignConfigToTidy;
  Result := tidyParseFile(FTidyDocument, PAnsiChar(AFileName)) >= 0;
end;
...
begin
  AssignConfigToTidy;
  Result := tidyParseString(FTidyDocument, PAnsiChar(AInputString)) >= 0;
end;
...
begin
  ...
  begin
    ...
    tidyOptSetValue(FTidyDocument, TidyDoctype, PAnsiChar(FDoctype));
    ...
    tidyOptSetValue(FTidyDocument, TidyAltText, PAnsiChar(FAltText));
    //tidyOptSetValue(FTidyDocument, TidySlideStyle, PAnsiChar(FSlideStyle));  // not used yet
    tidyOptSetValue(FTidyDocument, TidyErrFile, PAnsiChar(FErrFile));
    ...
    tidyOptSetValue(FTidyDocument, TidyEmacsFile, PAnsiChar(FEmacsFile));
    ...
    if FCSSPrefix <> '' then // if it is '', there is a configuration error in tidy !?
      tidyOptSetValue(FTidyDocument, TidyCSSPrefix, PAnsiChar(FCSSPrefix));
    tidyOptSetValue(FTidyDocument, TidyInlineTags, PAnsiChar(FInlineTags));
    tidyOptSetValue(FTidyDocument, TidyBlockTags, PAnsiChar(FBlockTags));
    tidyOptSetValue(FTidyDocument, TidyEmptyTags, PAnsiChar(FEmptyTags));
    tidyOptSetValue(FTidyDocument, TidyPreTags, PAnsiChar(FPreTags));
