...
uses
  ...
  ComCtrls, StdCtrls, ExtCtrls, Spin, ActiveX,
  {$IFDEF DELPHI_6_UP}
  ...
  VirtualShellNotifier, VirtualDataObject, ToolWin, VirtualShellToolBar,
...
type
  TWndSearchFiles = class(TForm)
    ...
    procedure SearchResultAddCustomShellColumn(Sender: TCustomVirtualExplorerTree;
...
var
  PreferredDropEffect: TPreferredDropEffect;
  DataObject: IDataObject;
begin
  PreferredDropEffect := TPreferredDropEffect.Create;
  try
...
begin
  ...
  Result := TVirtualDataObject.Create;
  HDrop := THDrop.Create;
...
procedure TWndSearchFiles.SearchResultAddCustomShellColumn(
...
begin
  for Index := 0 to Pred(SearchResult.Header.Columns.Count) do
  begin
    VETColumn := TVETColumn(SearchResult.Header.Columns.Items[Index]);
...
begin
  ...
  begin
    Handled := True;
    NSA := Sender.SelectedToNamespaceArray;
    Namespace.ShowPropertySheetMulti(NSA);
  end;
