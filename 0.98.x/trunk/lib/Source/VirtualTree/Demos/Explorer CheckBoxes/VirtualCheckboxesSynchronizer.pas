...
uses
  ...
  Dialogs, ShlObj, VirtualTrees, VirtualExplorerTree, VirtualShellUtilities,
  VirtualWideStrings, VirtualUnicodeDefines;
...
type
  ...
  public
    ...
    procedure GetCheckedFileNames(AStrings: TWideStringList; AllChecked: Boolean = False); overload;
    procedure GetCheckedFileNames(AStrings: TStrings; AllChecked: Boolean = False); overload;
    procedure SetCheckedFileNames(AStrings: TWideStringList); overload;
    procedure SetCheckedFileNames(AStrings: TStrings); overload;
    procedure GetResolvedFileNames(AStrings: TWideStringList); overload;
    procedure GetResolvedFileNames(AStrings: TStrings); overload;
...
function IsFolder(NS: TNamespace; IncludeMyComputer: Boolean = false;
  IncludeZipFolders: Boolean = false): Boolean;
...
procedure GetCheckedFileNames(VET: TCustomVirtualExplorerTree; AStrings: TWideStringList; AllChecked: Boolean = False); overload;
procedure GetCheckedFileNames(VET: TCustomVirtualExplorerTree; AStrings: TStrings; AllChecked: Boolean = False); overload;
...
begin
  Result := NS.Folder and
           (NS.FileSystem or (IncludeMyComputer and NS.IsMyComputer)) and
           (IncludeZipFolders or (WideUpperCase(NS.Extension) <> '.ZIP'));
end;
  ...
  begin
    ...
    begin
      if VET.ValidateNamespace(N, NS) then
        if StrICompW(PWideChar(NS.NameForParsing), PWideChar(Filename)) = 0 then
        begin
          Result := N;
...
begin
  Result := nil;
  if not Assigned(VET) or (not DirExistsW(Filename) and not FileExistsW(Filename)) then Exit;
...
procedure GetCheckedFileNames(VET: TCustomVirtualExplorerTree; AStrings: TWideStringList; AllChecked: Boolean = False);
...
begin
  ...
  AStrings.Assign(VET.Storage.CheckedFileNamesW);
end;
...
begin
  L := TWideStringList.Create;
  try
    GetCheckedFileNames(VET, L, AllChecked);
    AStrings.Assign(L);
  finally
    L.Free;
  end;
...
begin
  ...
  begin
    if IsFile(NS) then begin
      ...
      if IsFolder(NS, true) and (Node <> FTree.RootNode) and (foNonFolders in FTree.FileObjects) then
...
begin
  ...
  AllowAsChild := IsFile(Namespace) or IsFolder(Namespace, true);
...
procedure TVirtualCheckboxesSynchronizer.SetCheckedFileNames(AStrings: TWideStringList);
var
  I: integer;
  Node: PVirtualNode;
begin
...
begin
  ...
  try
    L.Assign(AStrings);
    SetCheckedFileNames(L);
  finally
    L.Free;
  end;
end;

procedure TVirtualCheckboxesSynchronizer.GetCheckedFileNames(AStrings: TWideStringList; AllChecked: Boolean = False);
begin
...
procedure TVirtualCheckboxesSynchronizer.GetResolvedFileNames(AStrings: TWideStringList);
begin
  AStrings.Assign(FTree.Storage.ResolvedFileNamesW);
end;
