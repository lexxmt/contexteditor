{
 SynEditDecorator (c) 2000 Daniel Parnell
 dparnell@oz.quest.com

 Feel free to use this in any way you like.  I would appeciate it if any changes could
 be sent back to me so they can be included in further versions.
}
...
begin
  if Value<>fLine then
    begin
      if fLine<>0 then
        TSynEditDecorator(TSynEditDecorations(Collection).GetOwner).Editor.InvalidateLine(fLine);
