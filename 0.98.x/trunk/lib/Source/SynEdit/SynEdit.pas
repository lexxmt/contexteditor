   ...
   {$DEFINE CODEFOLDING}
    ...
    {$ENDIF}
      ...
      protected
        procedure InsertBlock(const BB, BE: TBufferCoord; ChangeStr: PChar; AddToUndoList: Boolean); //!!EK
	      function GetAutoSizeDigitCount: Integer; //!!EK
    {$IFDEF SYN_CLX}
	      ...
	      begin
	        ...
	        begin
              ...
              //TSynCompletionProposal(fHint).ItemList.Add( StringReplace(
                ...
                delete(txt, StrLen(pchar(txt))-5, StrLen(pchar(txt)) );                                             //
   ...
   //###mod wrapglyph
	 ...
	 // {$ENDIF}
	    ...
	    begin
	      ...
	      Canvas.Pen.Color := CodeFolding.FolderBarLinesColor; //!!EK
  ...
  begin
    ...
    begin
      ...
      FoldRange := FoldRangeForLine(ToLine);
  		...
  		begin
  			CollapsedLines.AddObject(Lines[i], Lines.Objects[i]); //!!EK
  			Lines.Delete(i);
  		end;
  ...
  begin
  	...
  	begin
    	for i := 0 to CollapsedLines.Count - 1 do
      	Lines.InsertObject(FromLine, CollapsedLines[i], CollapsedLines.Objects[i]); //!!EK
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
        	if (aKey = VK_RETURN) and ((CaretX = 1) or (CaretX >= Length(Lines[CaretY - 1]) + 1)) then     //###mod caret beyond EOL
          	Result := True
          else if (aKey = VK_BACK) and (CaretX = 1) and (FoldRangeForLine(CaretY-1) = nil) then
          	Result := True
          else if(aKey in [16,17,18,27,112..123,33..46])then Result:=True //###mod collapsed line edit
          else
          	Result := False;
        end;
      end;
