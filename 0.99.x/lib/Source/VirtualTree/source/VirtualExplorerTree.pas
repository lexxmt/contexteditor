...
{*******************************************************************************}
  ...
  published
    ...
    property EncryptedTextColor: TColor index 3 read GetVETColor write SetVETColor default clGreen;
  end;
  ...
  then begin
    ...
    begin
      ...
      if (Length(S) = 3) and (S[2] = ':') and (S[3] = '\') and NS.Removable then
        Result := DiskInDrive(AnsiChar(string(S)[1]));
      if not Result then
      begin
        BeginUpdate;
        try
...
begin
 ...
 //    RebuildTree;  // JDK;   4/27/08 Removed, why am I doing this?
    finally
      WaitCursor(False);
...
begin
  ...
  begin
    ...
    try
      ...
      if Assigned(Node) then
        if ValidateNamespace(Node, NS) then
          NS.Delete(SelectedToNamespaceArray, ShiftKeyState)
    finally
      WaitCursor(False)
    end
  end
...
begin
  WideShowMessage(Application.Handle, S_PATH_ERROR, PWideChar(S_COMBOEDIT_DEFAULT_ERROR1 + InvalidText +
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        PIDL := PathToPIDL(WS, Application.MainForm.Handle);
        if Assigned(PIDL) then
        begin
          NS := TNamespace.Create(PIDL, nil);
