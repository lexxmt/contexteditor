...
type
  ...
  public
    ...
    function FindColorCode(FileName: WideString): TExtensionColorCode; overload;
    procedure Assign(Source: TPersistent); override;
    procedure Clear;
  ...
  protected
    ...
    property ExtensionColorCode: Boolean read FExtensionColorCode write SetExtensionColorCode;
...
procedure AddThumbRequest(Window: TWinControl; Item: TExplorerItem; ThumbSize: TPoint;
  UseExifThumbnail, UseExifOrientation, UseShellExtraction, UseSubsampling, IsResizing: Boolean;
  ThumbRequestClassCallback: TThumbThreadCreateProc);
...
begin
  ...
  FCompressedFile := TVirtualCustomFileTypes.Create(clBlue);
  FEncryptedFile :=  TVirtualCustomFileTypes.Create(clGreen);
    ...
    begin
      case GroupingColumn of
        0: begin  // Name Grouping
             if (NS.NameInFolder <> '') and (Groups[i].Caption <> '') then
             begin
...
begin
  Result := not (csDesigning in ComponentState) and (eloThreadedDetails in Options)
end;

function TCustomVirtualExplorerEasyListview.GetThreadedIconsEnabled: Boolean;
begin
  Result := not (csDesigning in ComponentState) and (eloThreadedImages in Options)
end;
...
begin
  Caption := TExplorerItem( Item).Namespace.NameForEditing
end;
...
begin
  ...
  if ExtensionColorCode and not Item.Selected and Assigned(TExplorerItem(Item).Namespace) then
  begin
    ColorCode := ExtensionColorCodeList.FindColorCode(TExplorerItem(Item).Namespace.NameForParsing);
...
procedure TCustomVirtualExplorerEasyListview.EnumThreadFinished;
var
  Item: TEasyItem;
begin
  BeginUpdate;
...
begin
  ...
  try
    ...
    if Assigned(Item) then
      Item.Namespace.Delete(SelectedToNamespaceArray, ShiftKeyState)
  finally
    Cursor := crDefault
  end
end;
...
begin
  ...
  try
    ...
    // If the path was a file then find the parent folder
      ...
      begin
        // Problems with the Postmessage so users wanted it changed to Sendmessage
        // but what was the point of Sendmessage?  Just remove and and change the root
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        DesktopFolder.Copy(SelectedToNamespaceArray)
      end else
      begin
        Item := TExplorerItem( Selection.First);
        if Assigned(Item) then
          Item.Namespace.Copy(SelectedToNamespaceArray)
      end
    end
  finally
    Cursor := crDefault;
...
begin
  ...
  try
    ...
    begin
      if AbsolutePIDLS <> 0 then
      begin
        if DesktopFolder.Cut(SelectedToNamespaceArray) and Mark then
          MarkSelectedCut
      end else
      begin
        Item := TExplorerItem( Selection.First);
        if Assigned(Item) then
        begin
           if Item.Namespace.Cut(SelectedToNamespaceArray) and Mark then
             MarkSelectedCut;
        end
      end
    end else
...
procedure TCustomVirtualExplorerEasyListview.DoEnumFolder(const Namespace: TNamespace;
...
begin
  if ValidateNamespace(Item, NS) and not(csDestroying in ComponentState) then
  begin
    if NS.States * [nsThreadedTileInfoLoaded, nsThreadedTileInfoLoading] = [] then
    begin
    ...
    begin
      ...
      if NSA[0].VerifyPIDLRelationship(NSA, True) then
        ...
        try
          ...
          begin
            APIDLList.Add( PIDLMgr.CopyPIDL(NSA[i].AbsolutePIDL));
            if NSA[i].FileSystem then
            begin
              {$IFDEF TNTSUPPORT}
               if IsUnicode then
                FileListW.Add(NSA[i].NameForParsing)
              else
                FileListA.Add(NSA[i].NameForParsing);
              {$ELSE}
              FileListA.Add(NSA[i].NameForParsing)
              {$ENDIF}
            end
      ...
      try
        ...
        begin
          ...
          begin
            ...
            Allow := True;
            NewNS := TNamespace.Create(PIDLMgr.CopyPIDL(PIDLArray[i]), RootFolderNamespace);
            if ((eloHideRecycleBin in Options) and NewNS.IsRecycleBin) or
              (NewNS.Parent.IsDesktop and (not IENamespaceShown and (NewNS.NameForParsing = IE_NAMEFORPARSING))) then
            begin
              NewNS.Free;
            end else
            begin
              ...
              begin
                if not Assigned(Result) then
                begin
                  Result := AddCustomItem(nil, NewNS, False);
                  if not Assigned(Result) then
                    NewNS.Free;
                end else
                begin
                  if AddCustomItem(nil, NewNS, False) = nil then
                    NewNS.Free;
                end
              end else
                NewNS.Free;
          ...
          begin
            ...
            GlobalThreadManager.FlushAllMessageCache(Self, TExplorerItem( ItemArray[j]));   // THE FIX OF THE CENTURY!!!!
            DoNamespaceStructureChange(TExplorerItem( ItemArray[j]), nscDelete);
            Groups.DeleteItem(ItemArray[j]);
            Inc(j)
          end
        end;
...
begin
  ...
  begin
    if (Value > -1) and (Value < Header.Columns.Count) then
    begin
...
begin
  ...
  begin
    FOptions := Value;
    ShowHint := eloQueryInfoHints in FOptions;
    Invalidate;
  end
end;
...
function TExtensionColorCodeList.FindColorCode(FileName: WideString): TExtensionColorCode;
var
  i, j: Integer;
...
begin
  Result := nil;
  {$IFDEF TNTSUPPORT}
  TargetExt := WideLowerCase(WideExtractFileExt(FileName));
  {$ELSE}
  TargetExt := WideLowerCase(ExtractFileExt(FileName));
  {$ENDIF}
  if TargetExt <> '' then
  begin
    i := 0;
...
begin
  {$IFDEF TNTSUPPORT}
  Extensions.CommaText := WideStringReplace(Value, ';', ',', [rfReplaceAll, rfIgnoreCase]);
  for i := 0 to Extensions.Count - 1 do
    Extensions[i] := Trim(WideExtractFileExt(Extensions[i]));
  {$ELSE}
  Extensions.CommaText := StringReplace(Value, ';', ',', [rfReplaceAll, rfIgnoreCase]);
  for i := 0 to Extensions.Count - 1 do
    Extensions[i] := Trim(ExtractFileExt(Extensions[i]));
  {$ENDIF}
  FExtensionMask := Value
end;
