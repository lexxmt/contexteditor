...
begin
  ...
  if ((HotKey shr 8) and HOTKEYF_SHIFT) <> 0 then
    Inc(Result, scShift);
  if ((HotKey shr 8) and HOTKEYF_CONTROL) <> 0 then
    Inc(Result, scCtrl);
  if ((HotKey shr 8) and HOTKEYF_ALT) <> 0 then
    Inc(Result, scAlt);
end;
...
begin
  ...
  if ShortCut and scShift <> 0 then
    Inc(Shift, HOTKEYF_SHIFT);
  if ShortCut and scCtrl <> 0 then
    Inc(Shift, HOTKEYF_CONTROL);
  if ShortCut and scAlt <> 0 then
    Inc(Shift, HOTKEYF_ALT);
...
begin
  ...
  if phe = nil then
    Exit;
  result := string(StrPas(phe^.h_name));
  WSACleanup;
end;
...
begin
  ...
  begin
    inc(loop);
    if CharInSet(StrData[loop], StripChars) then
...
begin
  ...
  begin
    if not CharInSet(Instr[count],['A'..'Z']) then
  ...
  begin
    if CharInSet(instr[1], ['A'..'Z']) and (instr[1] <> lchar) then
    begin
