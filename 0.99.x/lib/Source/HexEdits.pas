...
begin
  ...
  for I := 1 to Length(S) do
  begin
    if not CharInSet(S[I], ['0'..'9', 'A'..'F']) or
      (BaseStrToInt(S[I]) >= Base[NumBase]) then
      Result := False;
  end;
...
begin
  ...
  begin
    ...
    begin
      if not ValidPaste then
        Key := 0
    end
    else if Key = VK_DELETE then
    begin
      if not ValidCut then
        Key := 0
    end
  end
  ...
  begin
    if Key = VK_INSERT then
    begin
      if not ValidCopy then
        Key := 0
    end
    else if Key = VK_DELETE then
    begin
      if not ValidDelete then
        Key := 0
    end
  end
  else if Key = VK_DELETE then
...
procedure Register;
begin
  RegisterComponents('ConTEXT Components', [THexEdit]);
end;
