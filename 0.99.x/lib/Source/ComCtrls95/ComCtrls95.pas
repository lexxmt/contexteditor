...
type
  ...
  private
    ...
    fmouseovertab, fMouseDragTab, fMouseDownTab: integer;
...
{$ELSE}
  raise EListError.Create('TabAccessError' + #13#10 + 'Violation occured in ' + msg);
{$ENDIF}
end;
...
{$ELSE}
  AlignStyles: array[TTab95Position] of Cardinal = (0, TCS_BOTTOM);
{$ENDIF}
begin
  ...
  begin
    //Buttons or Tabs
    if (FButtonStyle) and (Style and TCS_Buttons = 0) then
      Style := Style or TCS_BUTTONS
    else if (not (FButtonStyle)) and (Style and TCS_Buttons <> 0) then
      Style := Style xor TCS_Buttons;
    //Normal or Vertical
    if (FVerticalTabs) and (Style and TCS_Vertical = 0) then
      Style := Style or TCS_VERTICAL
    else if (not (FVerticalTabs)) and (Style and TCS_Vertical <> 0) then
      Style := Style xor TCS_VERTICAL;
    //Tab positioning
    ...
    if ffocusbutton then
      style := style or TCS_FOCUSONBUTTONDOWN;
...
begin
{$IFNDEF VER90}
  if FDoubleBuffered and (Message.wParam <> Message.lParam) then
    Message.Result := 1
  else
{$ENDIF}
    inherited;
...
begin
  ...
  if Assigned(FOnDrawTab) then
    FOnDrawTab(Self, Caption, PageIndex, w1Rect)
  else
  begin
...
{$R-}
        if (iOriginal >= 0) and (iOriginal <= OriginalBMP.Width - 1) and
          (jOriginal >= 0) and (jOriginal <= OriginalBMP.Height - 1) then
        begin
...
{$R-}
          Mirror := 0;
          for J := OriginalBMP.Height - 1 downto 0 do
          begin
            RowOriginal := pTRGBArray(OriginalBMP.Scanline[Mirror]);
            Inc(Mirror);
            for I := OriginalBMP.Width - 1 downto 0 do
            begin
...
{$R-}
            if (iOriginal >= 0) and (iOriginal < OriginalBMP.Width) and
              (jOriginal >= 0) and (jOriginal < OriginalBMP.Height) then
            begin
              RowOriginal := pTRGBArray(OriginalBMP.Scanline[jOriginal]);
              Move(RowOriginal[iOriginal * BytesPP], RowRotated[i * BytesPP], BytesPP);
            end
            else
              FillChar(RowRotated[i * BytesPP], BytesPP, 192);
{$R+}
          end
        end;
      end;
...
{$ELSE}
    if itemID <> Cardinal(TabIndex) then
{$ENDIF}
    begin
...
begin
  ...
  fMouseDownTab := fMouseOverTab;
  inherited;
end;

procedure TTab95Control.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  if (fMouseDownTab = fMouseOverTab) then
  begin
    TabIndex := fMouseOverTab;
    if Assigned(FOnChange) then
      FOnChange(SELF);
  end;

  if (fTabShifting and (Button = mbLeft) and (fMouseOverTab <> -1) and (fMouseDownTab <> fMouseOverTab)) then
  begin
    Tabs.Move(fMouseDownTab, fMouseOverTab);
    Cursor := crDefault;
    if assigned(fOnTabShift) then
      fOnTabShift(self);
  end;
...
begin
  if (fTabShifting) and (ssLeft in Shift) and (fMouseOverTab <> fMouseDownTab) then
  begin
    if (fMouseOverTab = -1) then
      Cursor := crNo
    else if (fMouseDownTab <> fMouseOverTab) then
      Cursor := crDrag
    else
      Cursor := crDefault;
  end
  ...
  {
       if (ftabshifting) and (ssLeft in Shift) and (fMouseOverTab <> TabIndex) then
          fMouseDragTab := fMouseOverTab;
...
begin
  ...
  begin
    ...
    if SendMessage(FPageControl.Handle, TCM_GETITEM, WorkingIndex, Longint(@TCItem)) = 0 then
      TabControlError('TTab95Sheet.GetImageIndex');
    result := TCItem.iImage;
    if result <> fimageindex then
      result := fimageindex;
  end;
end;
...
begin
  ...
  if (Parent is TPage95Control) then
    if not PageControl.AllowPageToFloat then
      exit;
...
begin
  ...
  if (dragable) and not (FFloating) then
    ...
    begin
      ...
      ypos := 0;
      if gripalign = gabottom then
        ypos := (height - gripsize) + 2;
      if gripalign = garight then
        ypos := (width - gripsize) + 2;
      if (gripalign in [gabottom, gaTop]) then
        for loop := 0 to 4 do
        begin
          if gripalign = gaTop then
            position := (width - (xpos * loop)) - 4
          else
            position := xpos * loop;
...
begin
  for I := 0 to FPages.Count - 1 do
    TTab95Sheet(FPages[I]).FPageControl := nil;
  FPages.Free;
  inherited Destroy;
end;
...
begin
  if TabIndex >= 0 then
    SetActivePage(TTab95Sheet(Tabs.Objects[TabIndex]));
end;
...
begin
  ...
  begin
    ...
    begin
      if (fMouseOverTab = -1) then
        Cursor := crNo
      else if (fMouseDragTab <> fMouseOverTab) then
        Cursor := crDrag
      else
        Cursor := crDefault;
    end
