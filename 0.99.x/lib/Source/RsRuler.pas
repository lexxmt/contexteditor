...
type
  ...
  published
    ...
    property HairBlockWidth: integer read fHairBlockWidth write fHairBlockWidth;
    property ScaleColor;
    property TickColor;
...
procedure Register;
begin
  RegisterComponents('ConTEXT Components', [TRsRuler, TRsRulerCorner]);
end;
...
begin
  ...
  begin
    ...
    case fDirection of
      rdTop:
        case fHairLineStyle of
          hlsLine:
            R:=Rect(fHairLinePos - 1, Height - He - 1, fHairLinePos, Height - 8);
          hlsRect:
            begin
              if (fScaleDir=rsdNormal) then
                R:=Rect(1, Height - He - 1, fHairLinePos, Height - 8)
              else
                R:=Rect(Width, Height - He - 1, fHairLinePos, Height - 8);
            end;
          hlsBlock:
            R:=Rect(fHairLinePos-fHairBlockWidth, Height - He - 1, fHairLinePos, Height - 8);
        end;
      rdBottom:
        case fHairLineStyle of
          hlsLine:
            R:=Rect(fHairLinePos - 1, 8, fHairLinePos, He);
          hlsRect:
            begin
              if (fScaleDir=rsdNormal) then
                R:=Rect(1, 8, fHairLinePos, He + 1)
              else
                R:=Rect(Width, 8, fHairLinePos, He + 1);
            end;
        end;
      rdLeft:
        case fHairLineStyle of
          hlsLine:
            R:=Rect(Width - He, fHairLinePos - 1, Width - 8, fHairLinePos);
          hlsRect:
            begin
              if (fScaleDir=rsdNormal) then
                R:=Rect(Width - He, 1, Width - 8, fHairLinePos)
              else
                R:=Rect(Width - He, Height, Width - 8, fHairLinePos);
            end;
        end;
      rdRight:
        case fHairLineStyle of
          hlsLine:
            R:=Rect(8, fHairLinePos - 1, He, fHairLinePos);
          hlsRect:
            begin
              if (fScaleDir=rsdNormal) then
                R:=Rect(8, 1, He, fHairLinePos)
              else
                R:=Rect(8, Height, He, fHairLinePos);
            end;
        end;
    end;

    R:=Rect(R.Left+fLeftIndent, R.Top, R.Right+fLeftIndent, R.Bottom);
...
procedure TRsRuler.SetLeftOffset(const Value: integer);
begin
...
procedure TRsRuler.SetLeftIndent(const Value: integer);
begin
  if (fLeftIndent<>Value) then begin
    fLeftIndent:=Value;
    Invalidate;
  end;
end;
