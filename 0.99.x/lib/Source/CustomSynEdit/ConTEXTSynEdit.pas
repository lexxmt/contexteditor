...
////////////////////////////////////////////////////////////////////////////////////////////
//------------------------------------------------------------------------------------------
function TConTEXTSynEdit.GetAutoSizeDigitCount: Integer;
begin
...
procedure TConTEXTSynEdit.InsertBlock(const BB, BE: TBufferCoord; ChangeStr: PWideChar;
...
begin
  ...
  if SelAvail then
  try
    UseTabs:=(eoTabIndent in Options) and not (eoTabsToSpaces in Options);
...
begin
  if (fTrimTrailingSpaces) and (LineNumber>0) and (LineNumber<=Lines.Count) then begin
    ...
    len:=Length(s);
    if (len>0) and CharInSet(s[len],[' ', #09]) then
